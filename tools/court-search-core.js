...
export async function runJurisdictionSearch(p){
  const meta={backendConfigured:true,searchProviderConfigured:true,ddgFallbackActive:true,knownJurisdictionShortcutUsed:false,liveCrawlSucceeded:false};
  ...
  if(!seeds.length) return {...meta,backendConfigured:false,searchProviderConfigured:false,queries,results:[],errors:[...errors,'Search backend not configured'],fallbackCommand:`node tools/crawl-court-site.js --url=\"${p.startUrl||'https://examplecourt.gov/forms'}\" --state=${p.state||'ohio'} --county=${p.county||'wood-county'} --courtType=${p.courtType||'municipal'} --formCategory=${p.formCategory||'sealing-conviction'} --depth=${p.depth||2}`};
